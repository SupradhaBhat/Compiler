		...
		FILE *writeParsed=fopen("parsedTable.txt","w");
  		...
  		FILE *writeSymbol=fopen("symbolTable.txt","w");
    		fprintf(writeSymbol,"\n\t\tSymbolTable\n\nToken\tType\tLine Number\tScope\tFunction Number\n");
  			...
  			fprintf(writeSymbol,"\n%s\t\t%s\t\t%s\t\t%s\t\t%d\t  %d",ptr->token,ptr->type,ptr->line,ptr->scope,ptr->scopeValue,ptr->funcCount+1);
		}
...
//multiple variables in for, int i in for,dimension check for arrays,typedef
//accepts void main();
//structure attribute access [s.roll,s->roll]